...
#define COLOR_RAMP(colors, factor, finalColor) { int index=0; for (int i=0;i<2;i++){ ColorStop c=colors[i]; bool between=c.position<=factor; index=int(mix(float(index),float(i),float(between))); } ColorStop c=colors[index]; ColorStop n=colors[index+1]; float range=n.position-c.position; float lerpFactor=(factor-c.position)/range; finalColor=mix(c.color,n.color,lerpFactor); }
void main(){
  ...
  float intensity=0.6*height; float midPoint=0.20; float alpha=smoothstep(midPoint-uBlend*0.5,midPoint+uBlend*0.5,intensity);
  vec3 auroraColor=intensity*rampColor; fragColor=vec4(auroraColor*alpha,alpha);
}
`;
